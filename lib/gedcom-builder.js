...
function buildGedcomText(collections) {
  ...
  Object.values(individuals).filter(i => !i.deletedAt).forEach(indi => {
    ...
    (indi.multimediaRefs || []).forEach(mref => {
      ...
      (mm.files || []).forEach(f => {
        L.push('1 OBJE');
        if (f.form)          L.push(`2 FORM ${f.form}`);
        if (f.file)          L.push(`2 FILE ${f.file}`);
        if (f.title)         L.push(`2 TITL ${f.title}`);
        if (f.primary)       L.push('2 _PRIM Y');
        if (f.cutout)        L.push('2 _CUTOUT Y');
        if (f.parentRin)     L.push(`2 _PARENTRIN ${f.parentRin}`);
        if (f.personalPhoto) L.push('2 _PERSONALPHOTO Y');
        if (f.photoRin)      L.push(`2 _PHOTO_RIN ${f.photoRin}`);
        if (f.primaryCutout) L.push('2 _PRIM_CUTOUT Y');
        if (f.parentPhoto)   L.push('2 _PARENTPHOTO Y');
        if (f.position) {
          L.push(`2 _POSITION ${f.position}`);
        } else {
          const tag = (mm.tags || []).find(t => t.personId === indi.id && t.pixelCoords);
          if (tag) {
            const pc = tag.pixelCoords;
            L.push(`2 _POSITION ${pc.x1} ${pc.y1} ${pc.x2} ${pc.y2}`);
          }
        }
        if (f.note)          L.push(`2 NOTE ${f.note}`);
      });
    });
  });
