...
function parseGedcomToJson(text) {
  ...
  // Post-process: convert _POSITION data into tags[] entries so zones are visible in album
  for (const indiId of Object.keys(individuals)) {
    ...
    for (const mref of (indi.multimediaRefs || [])) {
      ...
      if (!mm) continue;
      if (!mm.tags) mm.tags = [];
      const alreadyTagged = mm.tags.some(t => t.personId === indiId);
      if (alreadyTagged) continue;

      // 1) Per-individual position (from INDI → OBJE @ref@ → _POSITION)
      const posStr = indiObjPositions[indiId] && indiObjPositions[indiId][mref];
      if (posStr) {
        const parts = posStr.trim().split(/\s+/).map(Number);
        if (parts.length === 4 && !parts.some(n => isNaN(n))) {
          const [x1, y1, x2, y2] = parts;
          mm.tags.push({ personId: indiId, personName, pixelCoords: { x1, y1, x2, y2 } });
        }
        continue;
      }

      // 2) Fallback: file-level position (inline OBJE or top-level OBJE with _POSITION)
  ...
  // Post-process: cross-link tags across multimedia entries sharing the same photoRin
  const byPhotoRin = {};
  ...
  for (const group of Object.values(byPhotoRin)) {
    ...
    // Collect all unique tags from every entry in the group
    const allTags = [];
    ...
    // Apply the full set of tags to each entry
    for (const mm of group) {
