...
function parseGedcomToJson(text) {
  ...
  // Merge CONC (concatenate) and CONT (new line) continuation lines into their parent
  const recs = [];
  let cur = null;
  for (const raw of lines) {
    ...
    if (lev === 0) {
      ...
      if (rest.startsWith('CONC ') || rest === 'CONC') {
        // CONC: append text directly to previous line's rest (no separator)
        const prev = cur.lines[cur.lines.length - 1];
        prev.rest += rest.replace(/^CONC ?/, '');
        prev.raw  += rest.replace(/^CONC ?/, '');
      } else if (rest.startsWith('CONT ') || rest === 'CONT') {
        // CONT: append text with newline separator
        const prev = cur.lines[cur.lines.length - 1];
        prev.rest += '\n' + rest.replace(/^CONT ?/, '');
        prev.raw  += '\n' + rest.replace(/^CONT ?/, '');
      } else {
    ...
    if (im) {
      ...
      for (let i = 1; i < rec.lines.length; i++) {
        ...
        if (rest.startsWith('NAME')) {
          ...
          const sm  = v.match(/\/(.*?)\//);
          const n   = { value: v, given: v.replace(/\/(.*?)\//, '').trim(), surname: sm ? sm[1].trim() : '', prefix: '', suffix: '', nickname: '', marriedName: '', aka: '', type: 'BIRTH' };
          subLines(rec.lines, i).forEach(s => {
            ...
            if (s.rest.startsWith('_MARNM')) n.marriedName = s.rest.replace(/^_MARNM\s*/, '').trim();
            if (s.rest.startsWith('_AKA'))  n.aka         = s.rest.replace(/^_AKA\s*/, '').trim();
          });
          indi.names.push(n);
        }
        ...
        for (const et of evTypes) {
          if (rest.startsWith(et + ' ') || rest === et) {
            const ev = { type: et, date: '', place: '', notes: [], cause: '', age: '', description: '' };
            subLines(rec.lines, i).forEach(s => {
              ...
              if (s.rest.startsWith('_CTRY')) ev.country     = s.rest.replace(/^_CTRY\s*/, '').trim();
              ...
              if (s.rest.startsWith('CAUS'))  ev.cause       = s.rest.replace(/^CAUS\s*/, '').trim();
              if (s.rest.startsWith('AGE'))   ev.age         = s.rest.replace(/^AGE\s*/, '').trim();
            });
            indi.events.push(ev);
          }
        }
        ...
        for (const at of atTypes) {
          if (rest.startsWith(at)) {
            ...
            subLines(rec.lines, i).forEach(s => {
              ...
              if (s.rest.startsWith('EMAIL')) attr.email = s.rest.replace(/^EMAIL\s*/, '').trim();
              if (s.rest.startsWith('WWW'))   attr.www   = s.rest.replace(/^WWW\s*/, '').trim();
              if (s.rest.startsWith('ADDR')) {
                attr.address = { addr: '', adr1: '', adr2: '', city: '', stae: '', ctry: '', post: '' };
                // ADDR may have a value on the same line
                const addrVal = s.rest.replace(/^ADDR\s*/, '').trim();
                if (addrVal) attr.address.addr = addrVal;
                // Parse ADDR sub-lines (level+1)
                const addrIdx = rec.lines.indexOf(s);
                if (addrIdx >= 0) {
                  for (let ai = addrIdx + 1; ai < rec.lines.length && rec.lines[ai].lev > s.lev; ai++) {
                    const ar = rec.lines[ai].rest;
                    if (ar.startsWith('ADR1')) attr.address.adr1 = ar.replace(/^ADR1\s*/, '').trim();
                    if (ar.startsWith('ADR2')) attr.address.adr2 = ar.replace(/^ADR2\s*/, '').trim();
                    if (ar.startsWith('CITY')) attr.address.city = ar.replace(/^CITY\s*/, '').trim();
                    if (ar.startsWith('STAE')) attr.address.stae = ar.replace(/^STAE\s*/, '').trim();
                    if (ar.startsWith('CTRY')) attr.address.ctry = ar.replace(/^CTRY\s*/, '').trim();
                    if (ar.startsWith('POST')) attr.address.post = ar.replace(/^POST\s*/, '').trim();
                  }
                }
              }
            });
        ...
        if (rest.startsWith('OBJE')) {
          ...
          if (m2) {
            ...
            const file = { file: '', form: '', title: '', position: null, cutout: false, primary: false, parentRin: '', parentPhoto: false, primaryCutout: false, personalPhoto: false, photoRin: '', note: '', photoDate: '', photoPlace: '' };
            subLines(rec.lines, i).forEach(s => {
              ...
              if (s.rest.startsWith('_DATE'))           file.photoDate     = s.rest.replace(/^_DATE\s*/, '').trim();
              if (s.rest.startsWith('_PLACE'))          file.photoPlace    = s.rest.replace(/^_PLACE\s*/, '').trim();
            });
            if (file.file) {
              ...
              if (file.photoRin && !file.cutout && photoRinToMid[file.photoRin]) {
                ...
                multimedia[existingMid].tags.push({
                  ...
                  pixelCoords: null,
                  objeProps: { position: file.position, cutout: file.cutout, primary: file.primary, parentRin: file.parentRin, parentPhoto: file.parentPhoto, primaryCutout: file.primaryCutout, personalPhoto: file.personalPhoto, photoRin: file.photoRin, note: file.note, photoDate: file.photoDate, photoPlace: file.photoPlace }
                });
              } else {
    ...
    if (om) {
      ...
      for (let i = 1; i < rec.lines.length; i++) {
        ...
        if (lev === 1 && rest.startsWith('FILE')) {
          fileObj = { file: rest.replace(/^FILE\s*/, '').trim(), form: '', title: '', position: null, cutout: false, primary: false, parentRin: '', parentPhoto: false, primaryCutout: false, personalPhoto: false, photoRin: '', note: '', photoDate: '', photoPlace: '' };
        ...
        if (lev === 2 && fileObj) {
          ...
          if (rest.startsWith('_DATE'))           fileObj.photoDate     = rest.replace(/^_DATE\s*/, '').trim();
          if (rest.startsWith('_PLACE'))          fileObj.photoPlace    = rest.replace(/^_PLACE\s*/, '').trim();
        }
